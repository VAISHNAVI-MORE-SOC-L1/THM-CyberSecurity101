# 🛡️ Cyber Security 101 – TryHackMe Learning Path

> A structured documentation of my hands-on cybersecurity learning journey through the **TryHackMe Cyber Security 101 Path**, covering networking, operating systems, web security, cryptography, offensive security, defensive security, and SOC fundamentals.

<img width="680" height="370" alt="image" src="https://github.com/user-attachments/assets/3776f84f-584f-4bb6-a2e5-aff2bc04aade" />



# 📊 Learning Statistics

- 🎯 **14 Learning Modules**
- 🧪 **56+ Hands-on Labs Completed**
- ✅ **200+ Lab Questions Solved**
- ⏱️ **45+ Hours of Practical Learning**
- 📝 **Room-by-Room Technical Documentation**
- 📸 **Screenshots & Notes Included**

---<img width="1680" height="738" alt="image" src="https://github.com/user-attachments/assets/378a4eb7-c71a-4ef2-99cf-ebed25af4c3c" />


# 📜 Overview

The **Cyber Security 101** pathway by TryHackMe is a beginner-friendly learning path designed to build a strong cybersecurity foundation through practical exercises and hands-on labs.

This repository serves as my personal technical notebook, documenting everything I learned while completing the path.

Inside this repository you'll find:

- Complete room documentation
- Practical lab walkthroughs
- Linux & Windows commands
- Networking notes
- Tool usage
- Cheatsheets
- Screenshots
- Lab summaries
- Key takeaways

> This repository focuses on technical documentation only. Personal insights and learning experiences are shared separately on LinkedIn and Medium.

---

# 🎯 Learning Objectives

This learning path helped me understand:

- Computer Networking
- Linux Fundamentals
- Windows Fundamentals
- Active Directory Basics
- Command Line (Linux, CMD & PowerShell)
- Cryptography
- Web Security
- Offensive Security
- Defensive Security
- Security Solutions
- SOC Fundamentals
- Malware Analysis Tools
- Cyber Security Career Fundamentals
- OWASP Top 10 (2025)

---

# 🗂️ Repository Structure

```text
THM-CyberSecurity101/
│
├── README.md
├── Certificate/
├── Notes/
│   ├── 01-Start-Your-Cyber-Security-Journey
│   ├── 02-Linux-Fundamentals
│   ├── 03-Windows-and-AD-Fundamentals
│   ├── 04-Command-Line
│   ├── 05-Networking
│   ├── 06-Cryptography
│   ├── 07-Exploitation-Basics
│   ├── 08-Web-Hacking
│   ├── 09-Offensive-Security-Tooling
│   ├── 10-Defensive-Security
│   ├── 11-Security-Solutions
│   ├── 12-Defensive-Security-Tooling
│   └── 13-Cyber-Career
│
├── Networking/
├── Resources/
├── Screenshots/
└── Certificate/
```

---

# 📚 Module Index

## 🔵 01 — Start Your Cyber Security Journey

- Offensive Security Intro
- Defensive Security Intro
- Search Skills

---

## 🐧 02 — Linux Fundamentals

- Linux Fundamentals Part 1
- Linux Fundamentals Part 2
- Linux Fundamentals Part 3

---

## 🪟 03 — Windows & AD Fundamentals

- Windows Fundamentals 1
- Windows Fundamentals 2
- Windows Fundamentals 3
- Active Directory Basics

---

## 💻 04 — Command Line

- Windows Command Line
- Windows PowerShell
- Linux Shell

---

## 🌐 05 — Networking

- Networking Concepts
- Networking Essentials
- Networking Core Protocols
- Secure Protocols
- Wireshark Basics
- Tcpdump Basics
- Nmap Basics

---

## 🔐 06 — Cryptography

- Cryptography Basics
- Public Key Cryptography
- Hashing Basics
- John the Ripper

---

## 💥 07 — Exploitation Basics

- Moniker Link (CVE-2024-21413)
- Metasploit Introduction
- Metasploit Exploitation
- Meterpreter
- Blue

---

## 🌍 08 — Web Hacking

- Web Application Basics
- JavaScript Essentials
- SQL Fundamentals
- Burp Suite Basics

---

## ⚔️ 09 — Offensive Security Tooling

- Hydra
- Gobuster
- Shells Overview
- SQLMap

---

## 🛡️ 10 — Defensive Security

- Defensive Security
- SOC Fundamentals
- Digital Forensics
- Incident Response
- Log Fundamentals

---

## 🔎 11 — Security Solutions

- SIEM
- Firewall
- IDS
- Vulnerability Scanners

---

## 🧰 12 — Defensive Security Tooling

- CyberChef
- CAPA
- REMnux
- FlareVM

---

## 💼 13 — Cyber Security Career

- Security Principles
- Careers in Cyber
- Training Impact on Teams

---

## 🚨 14 — OWASP Top 10 (2025)

- Identification & Authentication Failures
- Application Design Flaws
- Insecure Data Handling

---

# 🧪 Hands-on Labs

This repository documents every practical lab completed during the learning path.

Each room contains:

- Concept Explanation
- Important Notes
- Commands Used
- Screenshots
- Practical Exercises
- Output Analysis
- Troubleshooting
- Key Takeaways

---

# 🛠️ Tools & Technologies

### Platforms

- TryHackMe
- Kali Linux
- Ubuntu
- Windows

### Tools

- Bash
- PowerShell
- CMD
- Wireshark
- tcpdump
- Nmap
- Hydra
- Gobuster
- SQLMap
- Burp Suite
- CyberChef
- CAPA
- REMnux
- FlareVM

---

# 📖 Documentation Methodology

Every room follows the same structure to make revision easier.

1. Concept Overview

2. Commands & Syntax

3. Practical Lab Steps

4. Output Analysis

5. Security Insights

6. Room Summary

---

# 📜 Certificate

The completion certificate is available inside the **Certificate** directory.

<img width="1122" height="794" alt="image" src="https://github.com/user-attachments/assets/081e490f-0d02-4dc3-b172-6f78e78f5a1f" />


# 🚧 Repository Status

- ✅ Cyber Security 101 Path Completed
- ✅ Documentation In Progress
- 🔄 Repository Updated Regularly

---

# 🤝 Contributions

This repository is created for learning and knowledge sharing.

Suggestions, improvements, and feedback are always welcome.

---

